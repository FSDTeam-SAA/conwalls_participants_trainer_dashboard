...
import { Pencil, Trash2, Loader2, ChevronLeft, ChevronsRight, Users } from 'lucide-react'
...
export default function MeasureList({ projectId, stakeholder, onBack, onOverview }: MeasureListProps) {
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['measures', projectId, stakeholder._id] })
    ...
    if (isAdding || editingMeasure) {
        return (
            <MeasureForm
                ...
                onCancel={() => { setIsAdding(false); setEditingMeasure(null); }}
                onSuccess={() => { setIsAdding(false); setEditingMeasure(null); }}
            />
        )
    }
    ...
    return (
        <div className="space-y-6 pb-10">
            <h2 className="text-xl font-semibold text-[#00253E] mb-6">Measures for {stakeholder.name}</h2>
            ...
            <div className="space-y-0 border-b border-t border-gray-200 overflow-hidden">
                ...
                {measures.length === 0 ? (
                    ...
                    measures.map((m, index) => (
                        ...
                        >
                            <div className="font-semibold text-[#00253E] text-[16px] truncate">{m.name}</div>
                            <div className="text-gray-600 text-[16px]">{m.type}</div>
                            <div>
                                <span className={`px-4 py-1.5 rounded-full text-[14px] font-medium whitespace-nowrap ${getCategoryBadgeColor(m.category)}`}>
                                    {m.category || 'N/A'}
                                </span>
                            </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    {onOverview && (
                        <Button
                            ...
                            className="h-[48px] px-8 rounded-[4px] flex items-center gap-2 bg-[#B5CC2E] border-none text-[#00253E] hover:bg-[#A3B829] font-medium"
                        >
                            <span className="flex items-center gap-2">overview <ChevronsRight className="w-4 h-4" /></span>
                        </Button>
                    )}
                </div>

...
export default function TriggerForm({ stakeholder, onBack }: TriggerFormProps) {
    ...
    const getHelpText = (name: string) => {
        ...
        const help = helptexts.find((h: { name: string; values?: { en?: string } }) => h.name === name);
        return help?.values?.en || ''
    }

    const { register, handleSubmit, setValue, watch } = useForm<TriggerFormValues>({
        defaultValues: {
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['stakeholders', stakeholder.insightEngineId] })
            onBack()
        },
        onError: (err: Error) => {
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                ...
                <div className="space-y-3">
                    <div className="flex items-center gap-2">
                        <label className="text-[20px] font-medium text-[#00253E]">Benefits</label>
                ...
                <div className="space-y-3 pt-4">
                    <label className="text-[20px] font-medium text-[#00253E]">Trigger Evaluations</label>
                    <RadioGroup
                    ...
                    >
                        <div className="flex items-center space-x-2">
                            <RadioGroupItem value="LOW_POINTS" id="low" className="text-primary border-primary w-5 h-5" />
                            <label htmlFor="low" className="text-[18px] font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                                Low points
                            </label>
                        </div>
                        <div className="flex items-center space-x-2">
                            <RadioGroupItem value="HIGH_POINTS" id="high" className="text-primary border-primary w-5 h-5" />
                            <label htmlFor="high" className="text-[18px] font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                                High points
                            </label>
                        </div>
                    </RadioGroup>
                ...
                <div className="space-y-3 pt-4">
                    <div className="flex items-center gap-2">
                        <label className="text-[20px] font-medium text-[#00253E]">Objections / Concerns</label>
                ...
                <div className="space-y-3 pt-4">
                    <div className="flex items-center gap-2">
                        <label className="text-[20px] font-medium text-[#00253E]">Objection Handling</label>
